...
api_path = 'C:\\Program Files\\Lumerical\\v261\\api\\python\\lumapi.py'


import importlib.util
#default path for current release 
spec_win = importlib.util.spec_from_file_location('lumapi', 'C:\\Program Files\\Lumerical\\v261\\api\\python\\lumapi.py')
...
class Transceiver:
    ...
    def __init__(self):
        ...
        # Laser
        self.RIN = -160
        self.power_laser = 0.02  # 10 dBm - 10 mW -> It gets the values in W and transforms it into dBm
                                    # 20mW -> 13dBm -> HHI
    ...
    def transceiver_simple(self,
                           ...
                           v_pi_dc_tx=None,
                           v_pi_rf_tx=None,
                           v_bias_tx=None,  # at carrier suppression
                           v_pi_dc_rx=None,
                           v_pi_rf_rx=None,
                           v_bias_rx=None, # at quadrature
                           rf_freq1=None,
                           wg_enabled=None,
                           save_path=f"radar_results.pkl",
                           SOA_gain=None,
                           SOA_gain2=None,
                           NF_2=None,
                           gain_target_e=None
        ...
        """
        v_pi_dc_tx = v_pi_dc_tx if v_pi_dc_tx is not None else self.v_pi_dc_tx
        v_pi_rf_tx = v_pi_rf_tx if v_pi_rf_tx is not None else self.v_pi_rf_tx
        v_bias_tx = v_bias_tx if v_bias_tx is not None else self.v_bias_tx

        v_pi_dc_rx = v_pi_dc_rx if v_pi_dc_rx is not None else self.v_pi_dc_rx
        v_pi_rf_rx = v_pi_rf_rx if v_pi_rf_rx is not None else self.v_pi_rf_rx
        v_bias_rx = v_bias_rx if v_bias_rx is not None else self.v_bias_rx
        ...
        # ic.set("modulator type", "unbalanced single drive")
        ic.set("dc bias source", "internal")  # internal because I do not need extra components
        ic.set("bias voltage 1", v_bias_tx)  # Why is it 1? Because is cuadrature and the it divides by 2, and then
        # Lumerical counts it as you would have a bias voltage in 2 arms, and that is why
        # you have to divide it by 2 again
        ic.set("pi dc voltage", v_pi_dc_tx)
        ic.set("pi rf voltage", v_pi_rf_tx)
        ic.set("extinction ratio", 100)  # dB
        ic.set("insertion loss", 5)  # dB
        ic.set("phase shift", 0)  # rad
        ...
        # Optical Amplifier MOPA
        ic.addelement("Optical Amplifier")
        gain2 = "AMP_2"
        ic.set("name", gain2)
        ic.set("x position", 700)
        ic.set("y position", 0)
        ic.set("gain", SOA_gain2)
        ic.set("enable noise", True)
        ic.set("noise figure", NF_2)
        ic.set("enabled", True)
        ...
        ic.set("gain", gain_target_e)
        ic.set("noise parameter", "noise figure")  # before , "output"
        ic.set("noise figure", 3)  # dB
        ...
        # ic.set("modulator type", "unbalanced single drive")
        ic.set("dc bias source", "internal")  # internal because I do not need extra components
        ic.set("bias voltage 1", v_bias_rx)  # Why is it 1? Because is quadrature and then it divides by 2, and then
        # Lumerical counts it as you would have a bias voltage in 2 arms, and that is why
        # you have to divide it by 2 again
        ic.set("pi dc voltage", v_pi_dc_rx)
        ic.set("pi rf voltage", v_pi_rf_rx)
        ic.set("extinction ratio", 100)  # dB
        ic.set("insertion loss", 5)  # dB
        ic.set("phase shift", 0)  # rad
        ...
        ic.connect(gain1, "output", gain2, "input")
        ...
        # To MMI
        ic.connect(gain2, "output", mmi1x2_1, "input")
        ...
        ic.connect(bpf_target, 'output', t_delay, 'input')
        ic.connect(gain_target, 'input', t_delay, 'output')
        ic.connect(gain_target, 'output', att_elec, 'input')
        ...
        # Oscilloscope osc_pd_rx
        osc1_tx_res = ic.getresult(osc_pd_elec, "signal")
        osc1_tx = osc1_tx_res.get("amplitude (a.u.)")
        osc1_t_tx = osc1_tx_res["time"].ravel()
        #
        # # --------------------------------------------------
        ...
        #
        # Oscilloscope osc_pd_rx
        osc1_rx_res = ic.getresult(osc_pd_elec_rx, "signal")
        osc1_rx = osc1_rx_res.get("amplitude (a.u.)")
        osc1_t_rx = osc1_rx_res["time"].ravel()
        # # --------------------------------------------------
        # # Adapted network for the Photodiode
        # # Electrical Spectrum Analyzer - ESA_output
        # # Note: In here we have power units instead of amplitude
        ...
        entry = {
            "Electrical Power Spectrum Transmitter": {
                "Frequency (Hz)": freq_rf_pd,
                "Power (dBm)": P_rfsa_pd,
            },
            "Electrical Power Spectrum Receiver": {
                "Frequency (Hz)": freq_rf_pd_rx,
                "Power (dBm)": P_rfsa_pd_rx,
            },
            "Electrical Power Spectrum Transmitter Matched PD": {
                "Frequency (Hz)": freq_rf_out,
                "Power (dBm)": p1_rf_out,
            },
            "Electrical Power Spectrum Receiver Matched PD": {
                "Frequency (Hz)": freq_rf_out_rx,
                "Power (dBm)":  p1_rf_out_rx,
             },
            "Oscilloscope Transmitter":  {
                "Time (s)": osc1_t_tx,
                "Amplitude": osc1_tx
            },
            ...
            "Values": {
                "v_pi_dc_tx (V)": v_pi_dc_tx,
                "v_pi_rf_tx (V)": v_pi_rf_tx,
                "v_bias_tx (V)": v_bias_tx,
                "v_pi_dc_rx (V)": v_pi_dc_rx,
                "v_pi_rf_rx (V)": v_pi_rf_rx,
                "v_bias_rx (V)": v_bias_rx,
                "SOA_gain (dB)": SOA_gain
            },
        }
    ...
    def transceiver_gain_var(self,
                             ...
                             rf_freq1=None,
                             wg_enabled=None,
                             save_path=f"radar_gain_var.pkl",
                             SOA_gain=None,
                             SOA_gain2=None,
                             SOA_n=None,
                             NF_2=None,
        ...
        soa_g_values = np.linspace(soa_g_start, soa_g_stop, soa_g_n)
        # v_bias_values = np.linspace(v_bias_start, v_bias_stop, v_bias_n)

        results = {}

        for g_soa in soa_g_values:
            # ic.switchtodesign()

            # Change the gain of the SOA
            ic.setnamed(gain1, "gain", g_soa)
            ...
            # store by (v_pi, p_att)
            results[(float(g_soa))] = entry

            with open(save_path, "wb") as f:
                pickle.dump(entry, f, protocol=pickle.HIGHEST_PROTOCOL)

    def transceiver_gain_var_2target(self,
                                     R_in=None,
                                     v_pi_dc_tx=None,
                                     v_pi_rf_tx=None,
                                     v_bias_tx=None,  # at carrier suppression
        ...
        # Electrical time delay 2
        ic.addelement("Electrical Delay")
        t_delay2 = "DELAY_t2"
        ic.set("name", t_delay2)
        ic.set("x position", x_target + 1100)
        ic.set("y position", y_target)
        ic.set("delay", self.tau2)
...
rf_freq1 = 10e9
# tx.transceiver_simple( v_pi_dc_tx=None,
#                        v_pi_rf_tx=None,
#                        v_bias_tx=None,  # at carrier suppression
#                        v_pi_dc_rx=None,
#                        v_pi_rf_rx=None,
#                        v_bias_rx=None, # at quadrature
#                        rf_freq1=rf_freq1,
#                        save_path=f"radar_results_{rf_freq1/1e9}_GHz.pkl",
#                        SOA_gain=None,
#                        SOA_gain2=None,
#                        NF_2=None,
#                        gain_target_e=None
...
# Basically the idea is to run the simulation with the data you measured in your chip, unpackaged one
# for the RF modulator
# the tx MZM has to be for carrier suppression, so biased at null
# the rx is at quadrature
# the gains are the
# 1. as it is
